### Eve system parameter generation for a specified mass screen

generate_csv_file_for_mass_screen <- function(idmassscreen) {

 ## log that we are analysing this mass screen id
 timenow <- format(Sys.time(), "%Y %m %d %X")
 sink(logfile, append=T)
 string <- paste(timenow, " : Triggering analysis of mass screen id : ", idmassscreen, sep="")
 print(string)
 sink(NULL)

 avail_studies <- select_studies_for_mass_screen(idmassscreen)

 if(!is.list(avail_studies)) {
  timenow <- format(Sys.time(), "%Y %m %d %X")
  sink(logfile, append=T)
  print(paste(timenow, " : Failed to fetch study ids for mass screen id ",idmassscreen, sep=""))
  sink(NULL)
  return(F)
  ...
  if(!is.null(num_studies) && num_studies > 0) {
    ...
    # Fetch investigation name for mass screeb id for display
    investname = select_investname_for_mass_screen_id(idmassscreen)
    
    # select investigation id for mass screen id
    idinvestigation <- select_invest_id_for_mass_screen(idmassscreen)

    if(idinvestigation == -1) {
      ## log failure to select investigation ID for mass screen
      timenow <- format(Sys.time(), "%Y %m %d %X")
      sink(logfile, append=T)
      print(paste(timenow, " : Failed to select investigation ID for mass screen : ", idmassscreen, sep=""))
      sink(NULL)
      return(F)
    }

    # select cycle number for mass screen id
    cycle_number<- select_cycle_number_for_mass_screen(idmassscreen)

    if(cycle_number == -1) {
      ## log failure to select cycle number for mass screen
      timenow <- format(Sys.time(), "%Y %m %d %X")
      sink(logfile, append=T)
      print(paste(timenow, " : Failed to select cycle number for mass screen : ", idmassscreen, sep=""))
      sink(NULL)
      return(F)
    }
    ...
    csvfilename <- paste("MS_",idinvestigation,"_",cycle_number,"_",idmassscreen,"_",timestampnow,".csv",sep="")
    ...
    for (i in 1:num_studies){
      ...
      if(!is.list(avail_wells)) {
        ...
        if(!is.null(num_wells) && num_wells > 0) {
          ...
          timenow <- format(Sys.time(), "%Y %m %d %X")
          sink(logfile, append=T)
          print(paste(timenow, " : Number of wells zero for mass screen ID = ", idmassscreen, " and study ID = ",idstudy, sep=""))
      ...
      if (ussga == -1) {
        ...
        sink(NULL)
        print("EVE mass screen analysis trigger stopped after error updating study status to GEN ANALYSIS. Cannot continue.")
        return(F)
      }
    ...
    timenow <- format(Sys.time(), "%Y %m %d %X")
    sink(logfile, append=T)
    print(paste(timenow, " : Number of studies zero for mass screen ID = ", idmassscreen, sep=""))
    print(paste(timenow, " : Skipping processing this mass screen ID.",sep=""))
    sink(NULL)
    return(F)
