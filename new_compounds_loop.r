...
# 11 Mar: This version is being used for the loop of the first cherrypick 
#   simulation and run.
# 17 Mar: Fixed same bug as in non-loop version on this date
# 05 Apr: Added a routine to provide the compound IDs as a .xml file for feeding
#   back into Eve for the next round of cherrypicking; also added this as a 
#   .txt file as Ron seems to have difficulty with .xml files in his email.
#
################################################################################
#
# Input: Two files - the index output (chosenlist_loop.csv) from Kurt's Octave 
#      code (multiSelOptToFile.m) & the full index/compound list 
#      (all_compounds_indexed_loop.csv) from smiles.r
#
# Main outputs: 
#   1. A table (chosen_compounds_loop.csv), containing the index, the 
#       compound ID and the SMILES code
#   2. A .xml file for feeding back into Eve, containing the compound IDs
#       (as xml_list_loop.xml and xml_list_loop.txt)
#   
################################################################################

chosen_filename <- "chosenlist_loop.csv"
all_compounds_filename <- "all_compounds_indexed_loop.csv"
...
write.csv(chosen_all_data,file="chosen_compounds_loop.csv",row.names=FALSE)
# saves the output from the cherry-picking phase
...
write.table(data.matrix(xml_list),"xml_list_loop.xml",quote=FALSE,
                sep=",",col.names=FALSE,row.names=FALSE)
write.table(data.matrix(xml_list),"xml_list_loop.txt",quote=FALSE,
                sep=",",col.names=FALSE,row.names=FALSE)
